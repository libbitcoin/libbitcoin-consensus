 ...
 */
#ifndef LIBBITCOIN_CONSENSUS_TEST_SCRIPT_HPP
#define LIBBITCOIN_CONSENSUS_TEST_SCRIPT_HPP

// This file is copied from libbitcoin-system.
// Tests vectors are derived from satoshi sources.
//
// Textual encoding is modified in accordance libbitcoin style.
// We use a consistent approach to push data, which allows every opcode
// and related push data to be represented in a single token.
//
// Mixed case standard opcode names (system::chain::opcode_from_mnemonic).
// Multiple representations supported (e.g. nop2 and checklocktimeverify).
// 0x prefixed or decimal single byte tokens are numeric opcodes.
// 'ascii' tokens are converted to one byte per ascii character.
// [hex] tokens are hexidecimal, without the 0x prefix.
// [#.hex] tokens specify (by decimal #) the push data size (1, 2, or 4 bytes).
// This allows for non-minimal encoding with the four push data opcodes.
// All reserved and disabled codes are supported.
